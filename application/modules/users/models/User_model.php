...
class User_model extends CI_Model
{
    ...
    function getGroup()
    {
        ...
        $resultarray = array();
        if (isset($get['q'])) {
            $query = $this->db->query("SELECT `id`,`group_name` FROM tblgroups WHERE `group_name` LIKE '%{$get['q']}%' ORDER BY `group_name` ASC");
        } else {
            $query = $this->db->query("SELECT `id`,`group_name` FROM tblgroups ORDER BY `group_name` ASC");
        }
    ...
    public function edit_user($id)
    {
        ...
        $this->db->select($sql);
        $this->db->from("tblusers a");
        $this->db->join("tblgroups b", "a.group_id = b.id", "LEFT");
    ...
    public function processSuspendAccount($id)
    {
        ...
        $where = array("id" => $id);
        $updateResult = $this->db->update("tblusers", $data, $where);
    ...
    function resetSelectedUserPassword($user_id)
    {
        ...
        $username = $this->db
            ->get_where("tblusers users", array("id" => $user_id))
            ->row("username");
        ...
        if ($this->db->update("tblusers users")) {
            $resultSet["success"] = true;
    ...
    function processChangePassword($data)
    {
        ...
        $validatePin = $this->db->where(array("emp_id" => $id, "reset_pin" => $reset_pin))->count_all_results("tblusers");
        $this->db->reset_query();

        if ($validatePin >= 1) {
            $field = array("password" => MD5($data->password));
            $this->db->where(array("emp_id" => $id))->update("tblusers", $field);
    ...
    function processChangePin($data)
    {
        ...
        $validatePin = $this->db->where(array("emp_id" => $id, "reset_pin" => $old_pin))->count_all_results("tblusers");
        $this->db->reset_query();
    ...
    function verifyPin($data)
    {
        ...
        $validatePin = $this->db->where($where)->get("tblusers")->num_rows();
