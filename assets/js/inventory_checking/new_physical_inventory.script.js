...
setInterval(function () {
    $("#created_at").val(moment().format('MMM DD, YYYY hh:mm:ss A'));
}, 1000);
                        ...
                        `<tr id="row-${response[i].id}">
                           <td>
                               <label class='m-checkbox item-list-cb'>
                                   <input data-id="${response[i].id}" type="checkbox" class="form-control"
...
$.validate({
    form: '#frm-new-physical-inventory',
    lang: 'en',
    scrollToTopOnError: false,
    onSuccess: function (form) {
        const data = $(form).serialize();
        $.ajax({
            url: base_url + "inventory/save_physical_inventory",
            dataType: "JSON",
            type: "POST",
            data,
            beforeSend: function () {
            ...
            success: function (response) {
                ...
                if (response) {
                    toastr.success("Physical inventory successfully saved.", "Successfully saved.", {timeOut: 10000});
                } else {
                    toastr.error("An error occured while saving.", "Save Error.", {timeOut: 10000});
                }
